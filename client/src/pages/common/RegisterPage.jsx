...
function PasswordRow({ placeholder, show, toggle }) {
  return (
    <div
      className="flex items-center gap-2 px-3 py-2.5 rounded-xl w-full"
      style={{ background: "rgba(255,255,255,0.07)", border: "1px solid rgba(255,255,255,0.13)" }}
    >
      ...
      <input
        ...
        className="bg-transparent text-white placeholder-gray-400 text-sm w-full outline-none"
      />
      <button onClick={toggle} className="text-gray-400 hover:text-white transition-colors shrink-0">
        <EyeIcon show={show} />
      </button>
    </div>
  );
...
export default function RegisterPage() {
  ...
  const [accountType, setAccountType] = useState("searching");
    ...
    >
      ...
      <div className="relative z-10 w-full min-h-screen flex items-center justify-center px-4 py-8 sm:px-6 lg:px-10">
        <div className="w-full max-w-5xl flex flex-col lg:flex-row items-center justify-center lg:justify-between gap-8 lg:gap-12">
          ...
          {/* ── Register Card ─────────────────────────────── */}
          <div
            className="w-full max-w-[430px] rounded-2xl px-6 py-6 sm:px-8 sm:py-7 flex flex-col gap-3 shrink-0"
            style={{
          ...
          >
            ...
            <div className="flex items-center gap-2">
              <img
                src="/logo.png"
                alt="Aashraya"
                className="h-15 w-15 object-contain object-left"
                style={{ display: "block" }}
              />
            </div>
            ...
            <p className="text-gray-300 pl-2 text-xs sm:text-sm mt-0.5 leading-snug">
                Join Aashraya and discover verified rentals you can trust.
              </p>
...
<div className="grid grid-cols-2 gap-2 mb-1">
  ...
  <button
    type="button"
    onClick={() => setAccountType("searching")}
    className={`rounded-xl px-3 py-2 border transition-all duration-300 text-left ${
      accountType === "searching"
        ? "bg-green-500/15 border-green-400 shadow-md shadow-green-900/20"
        : "bg-white/[0.03] border-white/10 hover:border-green-400/40"
    }`}
  >
    <div className="flex items-center gap-2">
      <div
      ...
      >
        ...
        >
          <path
            ...
            strokeWidth={1.8}
            d="M15 15l6 6m-2-11a7 7 0 11-14 0 7 7 0 0114 0z"
          />
        </svg>
      </div>
      ...
      <div>
        ...
        <p className="text-[10px] text-gray-400 mt-1 leading-none">
          List property
            ...
            {/* Email */}
            <FieldRow
              icon={<svg className="w-4 h-4 text-gray-400 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>}
            ...
            {/* Password */}
            <PasswordRow placeholder="Password" show={showPw} toggle={() => setShowPw(!showPw)} />

            {/* Confirm Password */}
            <PasswordRow placeholder="Confirm Password" show={showCpw} toggle={() => setShowCpw(!showCpw)} />
            ...
            <label className="flex items-start gap-2 cursor-pointer">
              <input
                ...
                className="mt-0.5 w-3.5 h-3.5 accent-green-500 shrink-0"
              />
              <span className="text-gray-300 text-xs leading-snug">
                I agree to the{" "}
                <button className="text-green-400 hover:text-green-300 transition-colors">Terms of Service</button>
                {" "}and{" "}
            ...
            <p className="text-gray-400 text-xs text-center pt-1">
              ...
              <button onClick={() => navigate("/login")} className="text-green-400 hover:text-green-300 font-semibold transition-colors">
                Login
              </button>
            </p>
          </div>

          {/* ── Trust Badges (right side on lg, below on mobile) ── */}
